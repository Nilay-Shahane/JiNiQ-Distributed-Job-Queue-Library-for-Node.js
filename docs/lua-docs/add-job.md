...
5. `priorityOffset` (ms — how much of a head start priority jobs get; see [`claim-job.md`](./claimjob.md))
...
if maxQueueSize > 0 then
    local size = redis.call("ZCARD", normalKey) + redis.call("ZCARD", priorityKey)
...
Routing is mutually exclusive — a job is delayed, high-priority, or normal, never more than one.
